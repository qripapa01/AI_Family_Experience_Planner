...
// 핵심: 아이는 '재미있는 모험'을, 부모는 '앞으로 배울 교과와의 연결'을.
//       비교적 곧 배울 내용(가까운 시점)은 '지금이 좋은 시점(골든타임)'으로 강조한다.
...
function cardTemplate(item) {
  ...
  return `
    <article class="experience-card${golden ? ' golden' : ''}">
      ...
      <details>
        <summary>부모용 · 교과 연결 & 예상 학습 시기</summary>
...
function render() {
  ...
  // 가까운 시점(골든타임)을 먼저 보여준다
  filtered = filtered.slice().sort((a, b) => a.monthsAhead - b.monthsAhead);
